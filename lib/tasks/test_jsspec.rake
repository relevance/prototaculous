...
namespace :test do
  ...
  task :jsspec do
    raise "Specify the location of Rhino's js.jar in RHINO_HOME" unless ENV["RHINO_HOME"]
    Dir.chdir("test/javascripts") do
      all_fine = true
      if ENV["TEST"]
        all_fine = false unless system("java -jar #{ENV["RHINO_HOME"]}/js.jar #{ENV["TEST"]}")
      else
        Dir.glob("spec*.js").each do |file|
          all_fine = false unless system("java -jar #{ENV["RHINO_HOME"]}/js.jar #{file}")
        end
      end
